...
def _apply_method_group(state: State, method_group: str, logger) -> None:
    """方法组过滤: 将不在组内的 pending 方法标记为 skipped。
    ...
    for m in state.methods:
        if m.status == MethodStatus.PENDING and m.key.name not in group_names:
            m.status = MethodStatus.SKIPPED
            skipped_count += 1
    if skipped_count:
        logger.info(f"方法组过滤: 跳过 {skipped_count} 个非组内 pending 方法, "
                     f"组内共 {len(group_names)} 个方法名")
    # 重置计划快照, 强制重新生成(组内方法已变化)
    state.plan = None
