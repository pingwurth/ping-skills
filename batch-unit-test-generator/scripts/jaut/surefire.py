...
def aggregate_surefire_reports(project_root: str | Path,
                               modules: list[str]) -> TestResult:
    """遍历涉及模块各自的 surefire-reports, 合并为全模块 TestResult。

    fail-closed 语义保留: 任一模块报告缺失/不可解析/存在失败用例均按不绿计。
    单模块路径下退化为 parse_surefire_reports。
    """
    if len(modules) == 1:
        return parse_surefire_reports(project_root, modules[0])
    result = TestResult()
    for m in modules:
        mod_result = parse_surefire_reports(project_root, m)
        if mod_result.report_found:
            result.report_found = True
        result.tests += mod_result.tests
        result.failures += mod_result.failures
        result.errors += mod_result.errors
        result.skipped += mod_result.skipped
        result.parse_errors += mod_result.parse_errors
        result.failed_cases.extend(mod_result.failed_cases)
    result.assertion_failures = sum(1 for fc in result.failed_cases if fc.is_assertion)
    return result
