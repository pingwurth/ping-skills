    ...
    with store.locked():
        ...
        if not test_path.is_file():
            ...
            if streak >= config.VALIDATE_FAIL_STREAK_LIMIT:
                if _batch_auto_continue(state, workdir):
                    return _write_code(
                        f"测试类不存在(批量模式自动继续, 连续 {streak} 轮): {test_file}",
        ...
        if violations:
            ...
            if streak >= config.VALIDATE_FAIL_STREAK_LIMIT:
                if _batch_auto_continue(state, workdir):
                    ...
                    return _write_code(
                        f"规范校验未通过(批量模式自动继续, 连续 {streak} 轮): {len(violations)} 处违规",
...
def _batch_auto_continue(state: State | None, workdir: Path) -> bool:
    """批量模式下 validate 升级点是否可自动继续。

    state 为 None(仅凭 --test-file 校验)或 batch_mode=False 时返回 False;
    class_round_used >= BATCH_CLASS_ROUND_BUDGET 时返回 False(穿透至 ask_user)。
    """
    if state is None or not state.batch_mode:
        return False
    return state.class_round_used < config.batch_class_round_budget()
