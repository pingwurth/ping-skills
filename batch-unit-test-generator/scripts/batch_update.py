...
流程:
    ...
    → 落账(final_rate/rounds_used/escalations/attempts)
...
def handler(args: argparse.Namespace) -> tuple[Decision, EmitContext]:
    ...
    with batch_lock(batch_state_path):
        ...
        if class_state is None:
            ...
            if all_done:
                # 方法组模式: 当前组完成, 检查是否还有后续组
                if entry.has_method_groups:
                    has_more = entry.advance_group()
                    if has_more:
                        # 还有后续组, 标记为 pending 让 batch_next 再次认领
                        entry.status = "pending"
                        entry.claimed_at = None
                        entry.escalations = 0  # 重置升级计数
                ...
                # 可能有升级穿透(ask_user 未决)
                # 检查 state 是否处于 ask_user 等待态
                # 简化: 如果还有 pending 方法, 保持 in_progress
...
def _infer_escalation_reason(class_state) -> str:
    """从子代理 state.json 的轨迹中推断具体升级原因。

    按优先级检查(与 decisions.decide_after_verify 对齐):
        1. 类级轮次耗尽(BATCH_CLASS_ROUND_BUDGET)
        2. 当前方法迭代轮次耗尽(METHOD_ROUND_BUDGET)
        3. 连续测试失败不收敛(TEST_FAIL_STREAK_ROUNDS)
        4. 连续覆盖率无提升(NO_IMPROVEMENT_ROUNDS)

    Returns:
        描述升级原因的字符串(含句号前缀), 无法推断时返回空串。
    """
    # 1) 类级轮次耗尽
    if class_state.class_round_used >= config.batch_class_round_budget():
        return (f"原因: 类级迭代已达上限 {config.batch_class_round_budget()} 轮"
    ...
    # 2) 当前方法轮次耗尽
    if len(method.round_rates) >= config.METHOD_ROUND_BUDGET:
        return (f"原因: 方法 {current.label()} 已迭代 {len(method.round_rates)} 轮, "
                f"达单方法上限 {config.METHOD_ROUND_BUDGET} 轮"
                f"(当前覆盖率 {method.rate:.1f}%, 门槛 {class_state.threshold}%)。")

    # 3) 连续测试失败不收敛
    if len(method.round_test_results) >= config.TEST_FAIL_STREAK_ROUNDS:
        tail = method.round_test_results[-config.TEST_FAIL_STREAK_ROUNDS:]
        if all(r.failed for r in tail):
            trajectory = [(r.failures, r.errors) for r in tail]
            return (f"原因: 方法 {current.label()} 连续 {config.TEST_FAIL_STREAK_ROUNDS} 轮测试失败"
                    f"(每轮 Failures/Errors: {trajectory})。")

    # 4) 连续覆盖率无提升
    if len(method.round_rates) >= config.NO_IMPROVEMENT_ROUNDS:
        tail_rates = method.round_rates[-config.NO_IMPROVEMENT_ROUNDS:]
        trend_flat = all(tail_rates[i] <= tail_rates[i - 1] for i in range(1, len(tail_rates)))
        # 排除接近门槛的"已近达标"情况
        near_threshold = (class_state.threshold is not None
                          and max(method.round_rates) >= class_state.threshold
                            * config.NO_IMPROVEMENT_NEAR_THRESHOLD_FACTOR)
        if trend_flat and not near_threshold:
            return (f"原因: 方法 {current.label()} 连续 {config.NO_IMPROVEMENT_ROUNDS} 轮覆盖率无提升"
