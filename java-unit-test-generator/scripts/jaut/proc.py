...
class _ProgressReporter:
    ...
    def _is_process_alive(self) -> bool:
        ...
        if self._pid is None:
            return False
        try:
            os.kill(self._pid, 0)  # signal 0: 不发信号, 仅检查进程是否存在
            return True
        except OSError:
            return False
    ...
    def _run(self) -> None:
        while not self._stop.wait(config.MVN_PROGRESS_INTERVAL_SECONDS):
            # 进程已退出时停止报告, 避免"文件存在但进程不存在"的误导状态
    ...
    def _report(self) -> None:
        ...
        self._update_file(elapsed, status="running")

    def _report_exited(self) -> None:
        """进程已退出, 输出最终状态并更新进度文件。"""
        elapsed = int(time.time() - self._start_time)
        print(f"[mvn-progress] process exited after {elapsed}s", flush=True)
        self._update_file(elapsed, status="exited")

    def _update_file(self, elapsed: int, status: str = "running") -> None:
