...
def test_handler_baseline_mode_success(tmp_path):
    ...
    with patch("scripts.init_coverage.javasrc.resolve_target") as mock_resolve, \
         ...
         patch("scripts.init_coverage._ensure_test_file_exists", return_value=(False, Path("stub.java"))), \
         patch("scripts.init_coverage.maven.run_fast_single_cov") as mock_fast_cov, \
         patch("scripts.init_coverage.maven.run_mvn_test_with_jacoco") as mock_fallback, \
        ...
        mock_fast_cov.return_value = MagicMock(ok=True)
...
def test_handler_raises_when_target_class_excluded(tmp_path):
    ...
    with patch("scripts.init_coverage.javasrc.resolve_target",
         ...
         patch("scripts.init_coverage.maven.build_mvn_cmd") as mock_build, \
         patch("scripts.init_coverage.StateStore") as MockStore, \
         patch("scripts.init_coverage.setup_logger"):
        MockStore.return_value.locked.return_value.__enter__ = MagicMock(return_value=None)
        MockStore.return_value.locked.return_value.__exit__ = MagicMock(return_value=False)
...
def test_handler_raises_when_both_coverage_methods_fail(tmp_path):
    ...
    with patch("scripts.init_coverage.javasrc.resolve_target", return_value=("com.example.MyService", Path("src"))), \
         ...
         patch("scripts.init_coverage.maven.clean_surefire_dirs", return_value=[]), \
         patch("scripts.init_coverage._ensure_test_file_exists", return_value=(False, Path("stub.java"))), \
         patch("scripts.init_coverage.maven.run_fast_single_cov", return_value=MagicMock(ok=False)), \
         patch("scripts.init_coverage.maven.run_mvn_test_with_jacoco", return_value=MagicMock(ok=False)), \
         patch("scripts.init_coverage.StateStore") as MockStore, \
         patch("scripts.init_coverage.setup_logger"):
        MockStore.return_value.locked.return_value.__enter__ = MagicMock(return_value=None)
        MockStore.return_value.locked.return_value.__exit__ = MagicMock(return_value=False)
    ...
    assert exc_info.value.decision.exit_code == config.EXIT_ERROR
    assert "覆盖率采集失败" in exc_info.value.decision.summary


def test_handler_raises_when_mvn_fails(tmp_path):
    """mvn 执行失败时应抛出 StepError。"""
    args = _make_args(tmp_path)
    ...
    with patch("scripts.init_coverage.javasrc.resolve_target", return_value=("com.example.MyService", Path("src"))), \
         ...
         patch("scripts.init_coverage.StateStore") as MockStore, \
         patch("scripts.init_coverage.setup_logger"):
        mock_fast_cov.return_value = MagicMock(ok=False)
...
def _handler_patch_context(tmp_path, parsed_methods, class_rate: float = 100.0,
    ...
    patches = {
        ...
        "scripts.init_coverage._ensure_test_file_exists": (False, Path("stub.java")),
    ...
    mock_fast_cov = stack.enter_context(
        patch("scripts.init_coverage.maven.run_fast_single_cov",
              return_value=MagicMock(ok=True)))
    stack.enter_context(
        patch("scripts.init_coverage.maven.run_mvn_test_with_jacoco",
    ...
    MockStore.return_value.load.return_value = None
    MockStore.return_value.coverage_path.return_value = Path("coverage.json")
    return stack, mock_fast_cov, MockStore
