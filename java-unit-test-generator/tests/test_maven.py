...
def test_build_mvn_cmd_jacoco_configured(tmp_path):
    ...
    with patch("jaut.maven.resolve_tool", return_value="/usr/bin/mvn"):
        cmd = maven.build_mvn_cmd(tmp_path, test_classes=["FooTest"])
...
def test_build_mvn_cmd_multi_module_with_pl(tmp_path):
    _write_pom(tmp_path, _multi_module_pom())
    with patch("jaut.maven.resolve_tool", return_value="/usr/bin/mvn"):
        cmd = maven.build_mvn_cmd(tmp_path, test_classes=["FooTest"], module="module-a")
    assert cmd is not None
    assert "-pl" in cmd
    idx = cmd.index("-pl")
...
def test_build_mvn_cmd_root_module_no_pl(tmp_path):
    _write_pom(tmp_path, _multi_module_pom())
    with patch("jaut.maven.resolve_tool", return_value="/usr/bin/mvn"):
        cmd = maven.build_mvn_cmd(tmp_path, test_classes=["FooTest"], module=".")
    assert cmd is not None
    assert "-pl" not in cmd
...
def test_run_fast_single_cov_script_not_found(tmp_path):
    """脚本不存在时应返回 ok=False。"""
    # 将 __file__ 指向临时目录, 确保 jacoco/fast-single-cov.sh 不存在
...
# 降级场景集成
# --------------------------------------------------------------------------- #
def test_fallback_scenario_script_fails_then_mvn_succeeds(tmp_path):
    """降级场景: fast-single-cov.sh 失败 -> run_mvn_test_with_jacoco 成功。"""
    _write_pom(tmp_path, _single_module_pom(jacoco=True))

    # 模拟 fast-single-cov.sh 失败
    with patch("jaut.maven.run_fast_single_cov") as mock_fast, \
        ...
        # 模拟调用方的降级逻辑
        result = maven.run_fast_single_cov(
            tmp_path, ".", "com.example.Foo", "com.example.FooTest",
            str(tmp_path / "mvn.log"))
        if not result.ok:
            result = maven.run_mvn_test_with_jacoco(
                tmp_path, ".", "FooTest", "0.8.12", str(tmp_path / "mvn.log"))
...
def test_fallback_scenario_both_fail(tmp_path):
    """降级场景: 两种方案都失败 -> 最终 ok=False。"""
    _write_pom(tmp_path, _single_module_pom(jacoco=True))
...
def test_fallback_compilation_failure_skips_fallback(tmp_path):
    """编译失败 -> 跳过降级, 不调用 run_mvn_test_with_jacoco。"""
    _write_pom(tmp_path, _single_module_pom(jacoco=True))
    mvn_log = tmp_path / "mvn.log"
    mvn_log.write_text("❌ 编译失败！\n")
    ...
    with patch("jaut.maven.run_fast_single_cov") as mock_fast, \
        ...
        result = maven.run_fast_single_cov(
            tmp_path, ".", "com.example.Foo", "com.example.FooTest",
            str(mvn_log))
        if not result.ok and maven._is_fallbackable_failure(str(mvn_log)):
            result = maven.run_mvn_test_with_jacoco(
                tmp_path, ".", "FooTest", "0.8.12", str(mvn_log))
